    ...
    for name in df_filenames['filename']:
        ...
        for index, row in extracted_value.iterrows():
            df_result = df_result.append(
                {'filename': str('%010d' % (int(name[:-4]) - startid)) + '.jpg', 'file_size': row['file_size'],
...
def clean_up_incorrect_filename(file):
    df_original = pd.read_csv(file)
    df_original.dropna()
    new = df_original["filename"].str.split(",", expand=True)
    new['filename'] = new[0]
    new['file_size'] = new[1]
    new['file_attributes'] = new[[2, 3, 4]].agg(','.join, axis=1)
    new['region_count'] = new[5]
    new['region_id'] = new[6]
    new['region_shape_attributes'] = new[[7, 8, 9, 10, 11]].agg(','.join, axis=1)
    new['region_attributes'] = new[[12, 13, 14, 15]].agg(','.join, axis=1)
    result = new[['filename', 'file_size', 'file_attributes', 'region_count', 'region_id', 'region_shape_attributes',
                  'region_attributes']]
